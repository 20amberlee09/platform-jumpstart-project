...
const StepTrustName = ({ onNext, onPrev, data, updateStepData, currentStepKey }: StepTrustNameProps) => {
  ...
  const handleVerificationComplete = () => {
    console.log('Verification button clicked:', { trustName, ustpoChecked, stateChecked });
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex justify-between">
            ...
            <Button 
              onClick={() => {
                console.log('Continue clicked after verification:', { canProceed, isVerified });
                if (canProceed) {
                  onNext({ 
                    trustName,
                    ustpoChecked,
            ...
            >
              {canProceed ? "Continue to Minister Ordination" : "Complete Verification First"}
            </Button>
          </div>
          ...
          {process.env.NODE_ENV === 'development' && (
            <div className="text-xs text-gray-500 mt-2 p-2 bg-gray-100 rounded">
              <div>Debug: trustName={!!trustName.trim()}, ustpoChecked={ustpoChecked.toString()}, stateChecked={stateChecked.toString()}</div>
              <div>isVerified={isVerified.toString()}, canProceed={canProceed.toString()}, onPrev available: {!!onPrev}</div>
            </div>
          )}
    </div>
  );
